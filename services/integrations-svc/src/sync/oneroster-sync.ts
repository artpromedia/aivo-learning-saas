...
export class OneRosterSync {
  ...
  async deltaSync(
    ...
    try {
      const students = await this.fetchUsers(
        baseUrl,
        accessToken,
        "student",
      ...
      const changes = this.deltaDetector.detectChanges([], students);
  ...
  private async fetchUsers(baseUrl: string, accessToken: string, role: string, filter?: string): Promise<SisStudent[]> {
