...
import { subscribeEvent, LEARNING_SCHEMAS, ENGAGEMENT_SCHEMAS, BRAIN_SCHEMAS } from "@aivo/events";
...
export async function setupSubscribers(app: FastifyInstance): Promise<void> {
  ...
  // lesson.completed
  try {
    await subscribeEvent(nc, "lesson.completed", LEARNING_SCHEMAS["lesson.completed"], async (data) => {
  ...
  // quiz.completed
  try {
    await subscribeEvent(nc, "quiz.completed", LEARNING_SCHEMAS["quiz.completed"], async (data) => {
  ...
  // quiz.perfect_score
  try {
    await subscribeEvent(nc, "quiz.perfect_score", LEARNING_SCHEMAS["quiz.perfect_score"], async (data) => {
  ...
  // focus.session_30min
  try {
    await subscribeEvent(nc, "focus.session_30min", ENGAGEMENT_SCHEMAS["focus.session_30min"], async (data) => {
  ...
  // focus.session_90min
  try {
    await subscribeEvent(nc, "focus.session_90min", ENGAGEMENT_SCHEMAS["focus.session_90min"], async (data) => {
  ...
  // break.completed
  try {
    await subscribeEvent(nc, "break.completed", ENGAGEMENT_SCHEMAS["break.completed"], async (data) => {
  ...
  try {
    await subscribeEvent(nc, "engagement.streak.extended", ENGAGEMENT_SCHEMAS["engagement.streak.extended"], async (data) => {
  ...
  try {
    await subscribeEvent(nc, "engagement.challenge.completed", ENGAGEMENT_SCHEMAS["engagement.challenge.completed"], async (data) => {
