 ...
 *
 * 11 categories, ~49 questions. Includes routing weights and flag triggers
 * for automatic assessment mode determination.
 *
 * Ported from legacy aivo-platform baseline-svc.
 */

export interface ParentAssessmentQuestion {
  id: string;
  category: ParentAssessmentCategory;
  questionText: string;
...
export type ParentAssessmentCategory =
  | "learning_style"
  | "strengths"
  | "challenges"
  | "behavior"
...
export const CATEGORY_LABELS: Record<ParentAssessmentCategory, string> = {
  ...
  functioning_level: "Functioning Level",
  sensory_accessibility: "Sensory Accessibility",
  communication_needs: "Communication Needs",
  input_method: "Input Method",
  support_needs: "Support Needs",
};

export const CATEGORY_ORDER: ParentAssessmentCategory[] = [
  "learning_style",
  "strengths",
  "challenges",
  "behavior",
...
export const PARENT_ASSESSMENT_QUESTIONS: ParentAssessmentQuestion[] = [
  ...
  },
  {
    ...
    scaleMin: 1,
    scaleMax: 5,
    scaleLabels: { min: "Less than 5 minutes", max: "30+ minutes consistently" },
    required: true,
  },
  {
    id: "ls-4",
  ...
  },
  {
    ...
    scaleMin: 1,
    scaleMax: 5,
    scaleLabels: { min: "Prefers structured guidance", max: "Extremely imaginative and innovative" },
    required: true,
  },
  {
    id: "str-4",
    ...
    scaleMin: 1,
    scaleMax: 5,
    scaleLabels: { min: "Struggles to remember", max: "Excellent memory and recall" },
    required: true,
  },
  ...
  },
  {
    ...
    scaleMin: 1,
    scaleMax: 5,
    scaleLabels: { min: "Needs constant supervision", max: "Completely independent" },
    required: true,
  },
  {
    id: "ch-5",
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    scaleMin: 1,
    scaleMax: 5,
    scaleLabels: { min: "Requires significant encouragement", max: "Self-motivated and curious" },
    required: true,
  },
  {
    id: "beh-2",
  ...
  },
  {
    ...
    scaleMin: 1,
    scaleMax: 5,
    scaleLabels: { min: "Needs clear structure and routines", max: "Thrives with flexibility and choice" },
    required: true,
  },
  {
    id: "pref-3",
  ...
  // ══════════════════════════════════════════════════════════════════════════
  {
    ...
    scaleMin: 1,
    scaleMax: 5,
    scaleLabels: { min: "Low confidence, often doubts themselves", max: "Very confident and self-assured" },
    required: true,
  },
  {
    id: "se-2",
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // FUNCTIONING LEVEL (8 questions — critical for assessment routing)
  // ══════════════════════════════════════════════════════════════════════════
  {
    id: "fl-1",
  ...
  },
  {
    id: "fl-8",
    category: "functioning_level",
    questionText: "Which daily living skills can your child perform independently? (Select all that apply)",
    questionType: "multi_select",
    options: [
      "Feeding themselves",
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // SENSORY ACCESSIBILITY (5 questions)
  // ══════════════════════════════════════════════════════════════════════════
  {
    id: "sa-1",
  ...
  },
  {
    id: "sa-2",
    category: "sensory_accessibility",
    questionText: "If your child has vision impairment, what tools do they use? (Select all that apply)",
    questionType: "multi_select",
    options: [
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // COMMUNICATION NEEDS (3 questions)
  // ══════════════════════════════════════════════════════════════════════════
  {
    id: "cn-1",
  ...
  },
  {
    id: "cn-2",
    category: "communication_needs",
    questionText: "Does your child use an AAC (Augmentative and Alternative Communication) device or app?",
    questionType: "multiple_choice",
    options: [
      "No, does not use AAC",
      "Yes - Proloquo2Go",
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // INPUT METHOD NEEDS (3 questions)
  // ══════════════════════════════════════════════════════════════════════════
  {
    id: "im-1",
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // SUPPORT NEEDS DURING ASSESSMENT (5 questions)
  // ══════════════════════════════════════════════════════════════════════════
  {
    id: "sn-1",
  ...
  },
  {
    ...
    questionType: "open_ended",
    required: false,
    helpText: "Share any specific accommodations, equipment, or strategies that help your child succeed.",
  },
];

/** Get questions grouped by category */
export function getQuestionsByCategory() {
  const result: Record<string, ParentAssessmentQuestion[]> = {};
  for (const cat of CATEGORY_ORDER) {
    result[cat] = PARENT_ASSESSMENT_QUESTIONS.filter((q) => q.category === cat);
  }
  return result;
}

/** Get only accessibility-related categories (for quick routing) */
...
/** Get high-weight routing questions */
...
export function validateResponses(responses: Record<string, unknown>): {
  ...
  for (const question of requiredQuestions) {
    ...
    if (response === undefined || response === null || response === "") {
      errors.push(`Question "${question.id}" (${question.questionText.slice(0, 50)}) is required`);
    }
  }
