  ...
  async query(params: AuditLogQuery) {
    ...
    const [items, countResult] = await Promise.all([
      this.app.db
        ...
        .limit(limit)
        .offset(offset),
      this.app.sql`SELECT count(*)::int as total FROM admin_audit_logs ${
        whereClause
          ? this.app.sql`WHERE ${conditions.map(() => "true").join(" AND ")}`
          : this.app.sql``
      }`,
    ]);

    const total = countResult[0]?.total ?? items.length;
