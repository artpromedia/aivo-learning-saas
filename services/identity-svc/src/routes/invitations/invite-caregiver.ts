...
const inviteCaregiverBodySchema = z.object({
...
export async function inviteCaregiverRoute(app: FastifyInstance) {
  app.post(
    ...
    async (request, reply) => {
      ...
      const invitation = await invitationService.inviteCaregiver(
        request.user.sub,
        request.tenantId,
        body,
      );
      ...
      await emailService.sendTemplate("caregiver_invite", body.email, {
