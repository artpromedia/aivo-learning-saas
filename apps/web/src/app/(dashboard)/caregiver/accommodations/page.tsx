...
export default function CaregiverAccommodationsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const data = await apiFetch<{ accommodations: Accommodation[] }>("/api/caregiver/child/accommodations");
              ...
              <AnimatedCard key={acc.id} delay={400 + idx * 80}>
                <div className="flex items-start gap-4 p-4 rounded-xl" style={{ backgroundColor: "var(--aivo-bg)", border: "1px solid var(--aivo-border)" }}>
                  ...
                  <div className="flex-1">
                    ...
                    {acc.description && <p className="text-xs mt-0.5" style={{ color: "var(--aivo-text-secondary)" }}>{acc.description}</p>}
                    {acc.category && <span className="inline-block mt-2 px-2 py-0.5 rounded-full text-[10px] font-bold" style={{ backgroundColor: "var(--aivo-purple-50)", color: "#7C3AED" }}>{acc.category}</span>}
                  </div>
                </div>
              </AnimatedCard>
            ))}
