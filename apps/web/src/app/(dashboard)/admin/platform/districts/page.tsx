  ...
  if (loading) {
    return (
      <div className="space-y-6">
        ...
        <div className="space-y-3">{[1, 2, 3, 4].map((i) => (<Skeleton key={i} height={80} className="w-full rounded-3xl" />))}</div>
      </div>
    );
  }
    ...
    <PageWrapper>
      ...
      <PurpleGradientHeader className="rounded-3xl mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-extrabold">{t("districtsTitle")}</h1>
            <p className="text-white/80 text-sm">{t("districtsSubtitle")}</p>
          </div>
        </div>
      </PurpleGradientHeader>

      <div className="grid gap-3 grid-cols-3 mb-8">
        <StatCard icon={<Building2 size={18} />} label={t("statTotalDistricts")} value={districts.length} color="#3B82F6" delay={100} />
        <StatCard icon={<Users size={18} />} label={t("statTotalLearners")} value={districts.reduce((s, d) => s + d.learnerCount, 0).toLocaleString()} color="#7C3AED" delay={200} />
        <StatCard icon={<TrendingUp size={18} />} label={t("statAvgMastery")} value={`${Math.round(districts.reduce((s, d) => s + d.avgMastery, 0) / (districts.length || 1))}%`} color="#10B981" delay={300} />
      </div>
      ...
      <div className="mb-6">
        <div className="relative">
          ...
          <input
            ...
            placeholder={t("districtsSearchPlaceholder")}
      ...
      <ExpandableCard
        ...
        title={t("allDistrictsCount", { count: filtered.length })}
        subtitle={t("allDistrictsSubtitle")}
                ...
                <div className="flex items-center gap-4 p-4 rounded-xl cursor-pointer transition-all hover:scale-[1.01]" style={{ backgroundColor: "var(--aivo-bg)", border: "1px solid var(--aivo-border)" }}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 mt-1">
                      ...
                      <span className="text-xs" style={{ color: "var(--aivo-text-muted)" }}>{t("learnersCount", { count: district.learnerCount })}</span>
                      <span className="text-xs" style={{ color: "var(--aivo-text-muted)" }}>{t("teachersCount", { count: district.teacherCount })}</span>
                      <span className="text-xs font-bold" style={{ color: "#7C3AED" }}>{t("masteryPercent", { value: district.avgMastery })}</span>
                    </div>
                  </div>
