...
final _sessionsProvider =
  ...
  try {
    final res = await api.get(Endpoints.learningSessionHistory, queryParameters: {'learnerId': learnerId});
    return (res.data as List<dynamic>?)?.map((s) => s as Map<String, dynamic>).toList() ?? [];
  } catch (_) {
    return [
...
class TeacherLearnerSessionsScreen extends ConsumerWidget {
  const TeacherLearnerSessionsScreen({super.key, required this.learnerId});
                    ...
                    child: Row(
                      children: [
                        Container(
                          width: 40, height: 40,
                          decoration: BoxDecoration(
                            color: AivoColors.primary.withAlpha(26),
